...
<style>
                            ...
                            <div id="contact-form" class="ec-checkout-wrap margin-bottom-30">
                                <div class="ec-checkout-block ec-check-login border p-4 rounded">
                                    <h3 class="ec-checkout-title d-flex justify-content-between align-items-center">
                                        Contact
                                        <a href="javascript:void(0);" id="show-login-form" style="font-weight:600; color:blue; text-decoration: underline;">Login</a>
                                    </h3>
                                    <div class="ec-check-login-form">
                                        <form action="#" method="post">
                                            <div class="mb-3">
                                                <label>Email Address</label>
                                                <input type="email" name="email" id="contact-email" class="form-control" placeholder="Enter your email" required>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                                    ...
                                    <div class="ec-check-login-form">
                                        <form action="{{ route('login.submit') }}" method="post">
                                            ...
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <a class="text-decoration-none" style="font-weight:600" href="{{ route('register') }}">Create Account</a>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            ...
                            <div class="border p-4 rounded">
                                ...
                                <div class="accordion" id="accountAccordion">
                                    <div class="accordion-item" style="margin-bottom:20px">
                                        ...
                                        <div id="collapseOne" class="accordion-collapse collapse" data-bs-parent="#accountAccordion">
                                ...
                                <div class="ec-checkout-block ec-check-bill">
                                    ...
                                    <div class="ec-bl-block-content">
                                        ...
                                        <button type="button" class="btn btn-primary mt-5 mb-5"
                                            ...
                                            <button id="edit-delivery-address" class="btn btn-warning mt-5 mb-5" style="display: none;">Edit Delivery Address</button>
                                ...
                                <div class="shipping-method mb-2" id="payment_method_section" style="display: none;">
                                    ...
                                    <form action="">
                                        <div class="form-group radio-group">
                                            <input type="radio" id="braintree" name="payment_method" value="braintree"
                                            class="my-4"checked />
                                        <label for="braintree" class="d-block custom-label one mb-0 py-2 border mb-3">
                                            Credit Card
                                        </label>
                                            <!-- PayPal Option -->
                                            ...
                                            <!-- Credit Card (Braintree) Option -->


                                            <!-- Braintree Card Form (Hidden Initially) -->
                                            <div id="braintree-container" style="display: none;">
                                                <div id="braintree-error"
                                                    style="display: none; color: red; margin-top: 5px;"></div>

                                                <div id="dropin-loader" class="text-center py-3">
                                                    <span class="spinner-border text-primary" role="status"></span>
                                                    <p>Loading payment form...</p>
                                                </div>
                                                <div id="paypal-button-container"></div>
                                                <div id="dropin-container"></div> <!-- Keep it visible for Braintree -->
                                            </div>
                                        </div>
                                    </form>
                                ...
                                <div class="ec-checkout-pro">
                                    @if ($carts->isEmpty())
                                        @php
                                            $totalPrice = 0;
                                        @endphp
                                        <p class="no-cart-message">Your cart is empty. <a href="{{ route('all_products') }}">Shop now</a></p>
                                    @else
                                        @php
    ...
    $partIds = \App\Models\ProductPart::where('product_id', $product->product_id)->pluck('part_id');
    ...
    $maxServiceTime = $parts->where('part_type', 'Service')->max('delivery_time');
    $maxPhysicalTime = $parts->where('part_type', 'Physical')->max('delivery_time');

    // Sum the max delivery times
    $totalMaxDeliveryTime = ($maxServiceTime ?? 0) + ($maxPhysicalTime ?? 0)+7;
    ...
    $totalMaxDeliveryTimeInWeeks = round($totalMaxDeliveryTime / 7, 2);
                                                ...
                                                $packages = \App\Models\PackageProduct::where('packaging_product_id', $packagingProductId)->get();
                                                ...
                                                <div class="ec-product-inner">
                                                    ...
                                                    <div class="ec-pro-content">
                                                        <h5 class="ec-pro-title">
                                                            <a href="{{ route('product_details', ['slug' => $slug, 'id' => $product->product_id]) }}">{{ $product->product_id }}</a>
                                                        </h5>
                                                        <span class="ec-price">
                                ...
                                <div class="ec-checkout-summary">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-right" id="sub-total" data-value="{{ $subtotal }}">${{ number_format($subtotal, 2) }}</span>
                                    </div>
                                    <div>
                                        <span class="text-left">Sales Tax ({{ $rate->rate }}%)</span>
                                        <span class="text-right">${{ number_format($totalSalesTax, 2) }}</span>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <span id="shipping-charge" style="font-weight: bold; color: blue;" class="text-right">Enter full address</span>
                                    </div>
                                    <div>
                                        <span class="text-left">Total Amount</span>
                                        <span id="total-amount" class="text-right">${{ number_format($subtotal + $totalSalesTax, 2) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="ec-sb-block-content">
                                <div class="ec-check-pay-img-inner w-100">
                                    <div class="ec-check-pay-img mt-0">
                                        <img src="frontend/assets/images/icons/payment7.png" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('script')
   ...
   <script>
    ...
    <script>
        $(document).ready(function() {
              $("#braintree-container").slideDown();
            initializeBraintree();
            $('input[name="payment_method"]').change(function() {
                if ($(this).val() === "braintree") {
                    $("#braintree-container").slideDown();
                    initializeBraintree(); // Call function to load Braintree UI
                } else {
                    $("#braintree-container").slideUp();
                }
            });
        });

        var braintreeInstance = null; // Store globally

        function initializeBraintree() {
            if (braintreeInstance) {
                console.log("Braintree already initialized.");
                return; // Prevent duplicate initialization
            }

            $("#dropin-loader").show();
            $("#dropin-container").empty().show();
            ...
            $.get("{{ route('braintree.token') }}", function(data) {
                braintree.dropin.create({
                    ...
                    braintreeInstance = instance; // Store instance globally
                    console.log("Braintree UI Loaded");
                    $("#dropin-loader").hide();
                });
            }).fail(function() {
                alert("Error fetching Braintree token.");
            });
        }
    </script>
    <script>
        ...
        $(document).ready(function() {
            function fetchShippingCharge() {
                ...
                if (!state || !pinCode || !city || !address) {
                    ...
                    $("#total-amount").html(`$${parseFloat($("#sub-total").data("value")).toFixed(2)}`);
            ...
            function processBraintreePayment() {
                if (!braintreeInstance) {
                    alert("Braintree is not initialized. Please refresh and try again.");
                    return;
                }
                ...
                braintreeInstance.requestPaymentMethod(function(err, payload) {
                    ...
                    $.ajax({
                        url: "{{ route('braintree.checkout') }}",
                        type: "POST",
                        data: {
                            payment_method_nonce: payload.nonce,
                            total_amount: $("#total-amount").text().replace("$", "").trim(),
                            _token: "{{ csrf_token() }}"
                        },
                        beforeSend: function() {
                        ...
                        success: function(response) {
                            if (response.success) {
                                window.location.href =
                                    "{{ route('braintree.success') }}?transaction_id=" +
                response.transaction_id + "&amount=" + response.amount  + "&customer_email=" + response.customer_email ;
                            } else {
            ...
            $("#place-order-button").on("click", function(event) {
                ...
                if (selectedPaymentMethod === "paypal") {
                    ...
                    processBraintreePayment(); // Call Braintree function
                }
            });
